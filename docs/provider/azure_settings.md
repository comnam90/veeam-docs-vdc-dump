---
title: "Managing Settings"
product: "vdc"
doc_type: "provider"
source_url: "https://helpcenter.veeam.com/docs/vdc/provider/azure_settings.html"
last_updated: "6/2/2026"
product_version: ""
---

# Managing Settings


You can manage the Veeam service account and Azure SQL Server accounts, as well as view available storage.

In this section

* [Managing Veeam Service Account](azure_settings_accounts_service_view.md)
* [Managing SQL Server Accounts](azure_settings_accounts_database.md)
* [Viewing Veeam Data Cloud Repositories](azure_settings_storage.md)

