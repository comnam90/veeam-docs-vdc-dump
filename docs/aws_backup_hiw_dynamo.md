...
Veeam Data Cloud for AWS uses the [AWS Backup service](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/CreateBackupAWS.html) to create a cloud-native backup of a DynamoDB table and saves this backup to the specified backup vault within the AWS account to which the processed table belongs, in the AWS Region where the table resides. The backup is assigned AWS tags upon creation. Keys and values of AWS tags contain encrypted metadata that helps Veeam Data Cloud for AWS identify the related table backup.
