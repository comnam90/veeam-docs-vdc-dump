---
title: "Entra ID Restore"
product: "vdc"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vdc/userguide/entra_id_data_restore.html"
last_updated: "12/10/2025"
product_version: ""
---

# Entra ID Restore


Veeam Data Cloud offers the following types of data restore:

* [User restore](entra_id_user_restore.md) — to restore entire users or their properties.
* [Group restore](entra_id_group_restore.md) — to restore entire groups or their properties.
* [Administrative units restore](entra_id_units_restore.md) — to restore entire administrative units or their properties.
* [Role restore](entra_id_role_restore.md) — to restore entire roles or their properties.
* [Application restore](entra_id_app_restore.md) — to restore entire applications, service principals or their properties.
* [Conditional Access policies restore](entra_id_cap_restore.md) — to restore entire Conditional Access policies or their properties.
* [Microsoft Intune policies restore](entra_id_intune_restore.md) — to restore entire Microsoft Intune policies or their properties.
* [Log restore](entra_id_log_restore.md) — to restore audit and sign-in logs.
* [Object export](entra_id_object_export.md) — to export entire Entra ID objects to files in JSON format.

