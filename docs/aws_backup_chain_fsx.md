...
During every backup session, Veeam Data Cloud for AWS creates a cloud-native backup for each FSx file system added to the backup policy. To create the backup, Veeam Data Cloud for AWS uses the [AWS Backup service](https://docs.aws.amazon.com/aws-backup/latest/devguide/whatisbackup.html).
...
FSx backups act as independent restore points for backed-up file systems. If you remove any backup, it will not break the FSx backup chain — you will still be able to roll back file system data to any existing restore point.
