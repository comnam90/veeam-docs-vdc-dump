...
You can assign the following roles to users working with Veeam Data Cloud for AWS:

* AWS:Administrator — can add tenants and perform all configuration actions, backup and restore operations. You can limit access to specific AWS tenants when assigning this role.
* AWS:BackupOperator — can configure tenant settings, manage backup policies, manage protected data and perform all restore and archival operations. You can limit access to specific AWS tenants when assigning this role.
* AWS:RestoreOperator — can only perform restore operations. You can limit access to specific AWS tenants when assigning this role.
* AWS:Viewer — can monitor backup and restore processes without performing any operations. You can limit access to specific AWS tenants when assigning this role.

The following table describes the functionality available to users with different roles in Veeam Data Cloud. Note that users with AWS:Administrator, AWS:BackupOperator, AWS:RestoreOperator and AWS:Viewer roles assigned will have the described permissions only within the specified scope of tenants.

AWS Roles

| Role | AWS Tenant Management | AWS Backup | AWS Restore | AWS Tenant Settings |
| OrganizationAdmin | Full | Full | Full | Full |
| OrganizationViewer | View tenants | — | — | — |
| AWS:Administrator | Full | Full | Full | Full |
| AWS:BackupOperator | View tenants | Full | Full | Full |
| AWS:RestoreOperator | View tenants | View backup policies and sessions | Full | — |
| AWS:Viewer | View tenants | View backup policies and sessions | View restore sessions | — |
...
* EntraID:BackupAdministrator — can start backup policies and enable backup of additional Entra ID objects. You can limit access to specific Entra ID tenants when assigning this role.
...
The following table describes the functionality available to users with different roles in Veeam Data Cloud. Note that users with EntraID:Administrator, EntraID:SettingsAdministrator, EntraID:BackupAdministrator, EntraID:RestoreAdministrator and EntraID:Viewer roles assigned will have the described permissions only within the specified scope of tenants.

Entra ID Roles

| Role | Entra ID Tenant Management | Entra ID Backup | Entra ID Restore | Entra ID Tenant Settings |
| OrganizationAdmin | Full | Start backup policies | Full | Full |
| OrganizationViewer | View tenants | — | — | — |
| EntraID:Administrator | Full | Start backup policies | Full | Full |
| EntraID:SettingsAdministrator | View tenants | — | — | Full |
| EntraID:BackupAdministrator | View tenants | Start backup policies | — | Enable backup of additional objects |
| EntraID:RestoreAdministrator | View tenants | — | Full | — |
| EntraID:Viewer | View tenants | View backup policies and sessions | View restore sessions, backed-up objects and logs | — |
